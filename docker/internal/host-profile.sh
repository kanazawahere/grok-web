...
pi_web_docker_host_write_compose_override() {
  ...
  case "$host_profile" in
    ...
    *)
      printf '%s\n' "unsupported Grok Web Docker host profile: $host_profile" >&2
      return 1
      ;;
  esac

  cat >"$PI_WEB_DOCKER_HOST_OVERRIDE_TEMP" <<EOF
# Generated by Grok Web Docker host profile detection. Do not edit by hand.
...
EOF
  ...
  if [ -n "$control_path" ]; then
    if [ ! -e "$control_path" ]; then
      printf '%s\n' "Grok Web Docker control path does not exist: $control_path" >&2
      rm -f "$PI_WEB_DOCKER_HOST_OVERRIDE_TEMP"
      return 1
    fi
...
pi_web_docker_host_print_detection_failure() {
  printf '%s\n' "Grok Web Docker setup could not determine a supported host profile." >&2
  printf '%s\n' "" >&2
  printf '%s\n' "Detected:" >&2
