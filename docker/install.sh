...
Install or update the local-build Grok Web Docker runtime. The installer refreshes
...
temp_env=$env_file.$$
cat >"$temp_env" <<EOF
# Generated by the Grok Web Docker installer.
...
# npm package selection. Pi resolves from Grok Web's npm peer dependency.
...
log "Selected Grok Web Docker host profile: $pi_web_host_profile"
...
log "Persistent Grok Web Docker data: $data_dir"
...
log ""
log "WARNING: updating recreates the Grok Web Docker session daemon."
...
log "Recreating split Grok Web Docker services ..."
(
  cd "$install_dir"
...
log ""
log "Grok Web Docker runtime is ready: http://$pi_web_bind_addr:$pi_web_port"
